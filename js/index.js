...
/*! props where props are due - Terminal library by - terminal.js v2.0 | (c) 2014 Erik Österberg | https://github.com/eosterberg/terminaljs */
...
var terminalModal = document.getElementsByClassName('modal-content');
terminalModal[0].appendChild(t1.html);
		...
		info: [
			{
				...
				description:
					'MERN stack (MongoDB, Express, React, and Node.js) using password hashing for CRUD methods. Pulls movie/director information from MongoDB API. Allows you to change user information as well as favorite/remove movies once logged in. Hosted on Heroku.',
			...
			},
			{
				...
				description:
					'Discord bot that when given commands will: insult friends, praise friends, random gif, or search for a gif through various APIs. Uses OOP to query/update postgreSQL database for stats. Built with Node.js.',
			...
			},
			{
				...
				description:
					'Serverless PWA built with React using TDD/BDD/End-to-End testing.	Application uses the meetup API and AWS. Data visualization where applicable. Have to log into meetup to test. test email/pw email: kja76448@bcaoo.com	pw: testing',
...
const terminal = () => {
	t1.input('', function (input) {
		if (input == '~ ls') {
			...
			conditions = descriptions.map((project) => {
				return project.name;
			});
			...
			conditions = descriptions[1].info.map((project) => {
				return project.projectName;
			}); //returning all projectNames
			...
			//nothing matches, returns the terminal
			terminal();
		}
	});
};
